...
if __name__ == "__main__":
    ...
    select = ["Arrival_time_ave：", "Pcap_len", "Packet_len_ave", "Packet_num", "Cert_not_valid_before",
              "Cert_not_valid_after", "Cert_length", "Cipher_suite"]
    Features = Features[select]

    Label = network_traffic['Label']


    # result = pd.concat([Features, Label], axis=1)
    #
    # result.to_csv("保留八个特征.csv", index = None)
    test_size = 0.1
